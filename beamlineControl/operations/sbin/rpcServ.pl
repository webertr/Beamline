...
sub displayStart {
	my $retval = ` /usr/local/cnts/epics/operations/etc/init.d/statusdisplay start 2>&1 `;
	return $retval;
}
sub displayStop {
	my $retval = ` /usr/local/cnts/epics/operations/etc/init.d/statusdisplay stop 2>&1 `;
	return $retval;
}
